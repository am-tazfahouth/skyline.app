...
void main() {
  ...
  testWidgets('GPS action dispatches DetectCurrentLocationEvent', (
    tester,
  ) async {
    bloc = createBloc();
  ...
  testWidgets('GPS error keeps the favorites list visible', (tester) async {
    ...
    ).thenThrow(Exception('gps failed'));
    bloc.add(const LoadFavoritesEvent());
  ...
  testWidgets(
    'service disabled SnackBar offers Enable action opening location settings',
    (tester) async {
      bloc = createBloc();
      when(() => repo.loadFavorites()).thenReturn([]);
      when(
  ...
  testWidgets(
    'permanently denied SnackBar offers Settings action opening app settings',
    (tester) async {
      bloc = createBloc();
      when(() => repo.loadFavorites()).thenReturn([]);
      when(
  ...
  testWidgets(
    'permission denied SnackBar offers Retry action re-detecting location',
    (tester) async {
      bloc = createBloc();
      when(() => repo.loadFavorites()).thenReturn([]);
      when(
